...
class KokoClone:
    ...
    def _get_vocab_config(self, lang):
        ...
        if lang in {"zh", "ja"}:
            zh_vocab = os.path.join(ROOT_DIR, "model", "config-v1.1-zh.json")
            if not os.path.exists(zh_vocab):
                ...
                downloaded = os.path.join(ROOT_DIR, "config.json")
        ...
        local_config = os.path.join(ROOT_DIR, "model", "config.json")
    ...
    def _ensure_file(self, folder, filename):
        ...
        if not os.path.exists(filepath):
            ...
            hf_hub_download(
                ...
                local_dir=ROOT_DIR # Downloads securely into local ./model or ./voice
            )
        return filepath
