...
def _sec() -> str:
    return os.environ.get("MAGICVOICE_LICENSE_GUARD_SECRET", "")

def _url() -> str:
    return os.environ.get("MAGICVOICE_LICENSE_GUARD_URL", "")
...
def _check_online(username: str):
    ...
    import requests
    if not _url() or not _sec():
        raise RuntimeError("MagicVoice license guard is not configured")
    mid = _get_machine_id()
